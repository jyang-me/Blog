# 关于我

你好，欢迎来到我的博客。

我目前聚焦于嵌入式软件开发，会做一些前端工程实践，会分享一些大模型的配置方式，通过项目驱动学习，持续沉淀可复用的技术方法。在这里我会分享一些零基础学习的路线与经验，让大家少走弯路。

## 技术方向

- 嵌入式软件开发(stm32/rtos/linux)
- Web 前端工程（Vue / VitePress / TypeScript）
- 自动化工具与部署实践（Cloudflare）
- ai 相关技术（ChatGPT、CC 等）

## 联系方式

- GitHub: [github.com/ljyang009-prog](https://github.com/ljyang009-prog)
- Email: [ljyang009@gmail.com](mailto:ljyang009@gmail.com)

## 博客说明

本站基于 VitePress 构建，部署在 Cloudflare Pages。

你可以在这里看到我的学习路径、技术栈与技术文章输出。

---

最后更新：2026-03-28
